...
## アプリケーションを作成した背景
これまで自宅学習を進めていく中で、仮説検証して調べても解決できない問題に直面し、学習の手が止まってしまった経験があります。
その時に、解決できない問題を相談できるコミュニティーがあれば、より効率良く学習を進められるのではないかと考え、質問投稿アプリを作成しました。
このアプリを通して課題を解決するだけでなく、同じように受験勉強や資格取得などの目標に向けて勉強している仲間と繋がることで、学習意欲やモチベーション向上にも貢献できるのではないかと考えました。
今後は検索機能も実装していき、自分が解決したい課題について探しやすくできればと考えています。
...
# 工夫したポイント
タグをつけることにより、課題の関連性を可視化できるようにしました。tagsテーブルに同じタグが存在していた場合には新しく保存をすることはなく同じタグが存在しない場合のみtagsテーブル内に新しく保存されるようにしました。
